...
#以下是ANN-MLP交叉验证的函数
def rf_cv(hidden_layer_sizes,batch_size,max_iter):
    val = cross_val_score(MLPRegressor(hidden_layer_sizes=(int(hidden_layer_sizes),int(hidden_layer_sizes)),
...
file = open(r'/ws_correct_ML/machinelearning/mlp_parameter_win.txt', mode='w')
file.write('{:}'.format(rf_bo.res[max_index]))      
file.close()



file = open(r'/ws_correct_ML/machinelearning/mlp_parameter_win.txt', mode='r').read()
...
def DealData(X,Y,stationName):
   #结果列表
    ...
    pd.DataFrame({'TrueData':trues_train.ravel(),'MlpData':forest_train}).to_csv('./mlp训练ALL.csv')
    pd.DataFrame({'TrueData':trues_test.ravel(),'MlpData':forest_test}).to_csv('./mlp验证ALL.csv')
    ...
    for index in range(0,5):
        ...
        dataframe3 = pd.DataFrame({'TrueData':train_true.ravel(),'MlpData':train_forest})
        ...
        dataframe4 = pd.DataFrame({'TrueData':test_true.ravel(),'MlpData':test_forest})
...
# 保存模型 一个文件一个模型 ，注意使用
    with open('/ws_correct_ML/machinelearning/mlpmodel.pickle', 'wb+') as f:#
        pickle.dump(model, f)
    print("保存模型成功")
...
if __name__=='__main__':
    ...
    thread1.start()
    DealData(Xtrain, Ytrain, 'mlptrain')
    # 保存程序日志 
    plog="程序结束："+ str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) 
    ProgramLog.append(plog)
    logpath = fileSavePath+"/MLP程序日志.txt"
...
#模型保存后，从此处开始运行
with open('/ws_correct_ML/machinelearning/mlpmodel.pickle', 'rb') as f:
    model = pickle.load(f)
print("加载模型成功")
...
# # rf_metrics_DS= get_regression_metrics('Random Forest Regression', Y_DS1.ravel(), Y_DS_predict)
# # rf_metrics_test
# # rf_metrics_train
# # rf_metrics_DS
def RMSE(obs, pre):
...
data_train=pd.read_csv('/ws_correct_ML/machinelearning/mlp训练ALL.csv') 
data_valid=pd.read_csv('/ws_correct_ML/machinelearning/mlp验证ALL.csv')
Ytrain=data_train['TrueData']
Ytrain_predict=data_train['MlpData']
valid=data_valid['TrueData']
valid_predict=data_valid['MlpData']
ds_DS=pd.DataFrame(np.concatenate((Y_DS1, Y_DS_predict[:,np.newaxis]),axis=1),columns=['Y_DS1', 'Y_DS_predict'])
ds_DS.index=ds_test.index
ds_DS.to_csv('./testandmlp.csv')
ds_DS=pd.read_csv('/ws_correct_ML/machinelearning/testandmlp.csv')
...
plotreg(Ytrain, Ytrain_predict,valid, valid_predict,ds_test0,ds_DS0,ds_test1,ds_DS1,label='MLP',dir_save="/ws_correct_ML/machinelearning/MLPplotreg.png")
