...
# 以下是xgboost交叉验证的函数
def rf_cv(n_estimators, max_depth,gamma,min_child_weight):
    val = cross_val_score(XGBRegressor(n_estimators=int(n_estimators),
...
file = open(r'/ws_correct_ML/machinelearning/xgb_parameter_win.txt', mode='w')
file.write('{:}'.format(rf_bo.res[max_index]))      
file.close()


file = open(r'/ws_correct_ML/machinelearning/xgb_parameter_win.txt', mode='r').read()
...
def DealData(X,Y,stationName):
   #结果列表
    ...
    pd.DataFrame({'TrueData':trues_train.ravel(),'XgbData':forest_train}).to_csv('./xgb训练ALL.csv')
    pd.DataFrame({'TrueData':trues_test.ravel(),'XgbData':forest_test}).to_csv('./xgb验证ALL.csv')
    ...
    for index in range(0,5):
        ...
        dataframe3 = pd.DataFrame({'TrueData':train_true.ravel(),'XgbData':train_forest})
        ...
        dataframe4 = pd.DataFrame({'TrueData':test_true.ravel(),'XgbData':test_forest})
...
# 保存模型 一个文件一个模型 ，注意使用
    with open('/ws_correct_ML/machinelearning/xgbmodel.pickle', 'wb+') as f:#
        pickle.dump(model, f)
    print("保存模型成功")
...
if __name__=='__main__':
    ...
    thread1.start()
    DealData(Xtrain, Ytrain, 'xgbtrain')
    # 保存程序日志 
    plog="程序结束："+ str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) 
    ProgramLog.append(plog)
    logpath = fileSavePath+"/XGB程序日志.txt"
...
模型保存后，从此处开始运行
with open('/ws_correct_ML/machinelearning/xgbmodel.pickle', 'rb') as f:
    model = pickle.load(f)
print("加载模型成功")
...
data_train=pd.read_csv('/ws_correct_ML/machinelearning/xgb训练ALL.csv') 
data_valid=pd.read_csv('/ws_correct_ML/machinelearning/xgb验证ALL.csv')
Ytrain=data_train['TrueData']
Ytrain_predict=data_train['XgbData']
valid=data_valid['TrueData']
valid_predict=data_valid['XgbData']
...
ds_DS.to_csv('/ws_correct_ML/machinelearning/testandxgb.csv')
ds_DS=pd.read_csv('/ws_correct_ML/machinelearning/testandxgb.csv')
...
plotreg(Ytrain, Ytrain_predict,valid, valid_predict,ds_test0,ds_DS0,ds_test1,ds_DS1,label='XGBoost',dir_save='/ws_correct_ML/machinelearning/XGBplotreg.png')