...
file = open(r'/ws_correct_ML/machinelearning/rf_parameter_win.txt', mode='w')
file.write('{:}'.format(rf_bo.res[max_index]))      
file.close()


file = open(r'/ws_correct_ML/machinelearning/rf_parameter_win.txt', mode='r').read()
...
# 保存模型 一个文件一个模型 ，注意使用
    with open('/ws_correct_ML/machinelearning/rfmodel.pickle', 'wb+') as f:#
        pickle.dump(model, f)
    print("保存模型成功")
...
with open('/ws_correct_ML/machinelearning/rfmodel.pickle', 'rb') as f:
    model = pickle.load(f)
print("加载模型成功")
...
# plt.savefig("/ws_correct_ML/machinelearning/plot_features_weights.png")
...
data_train=pd.read_csv('/ws_correct_ML/machinelearning/rf训练ALL.csv') 
data_valid=pd.read_csv('/ws_correct_ML/machinelearning/rf验证ALL.csv')
...
ds_DS.to_csv('/ws_correct_ML/machinelearning/testandrf.csv')
ds_DS=pd.read_csv('/ws_correct_ML/machinelearning/testandrf.csv')
...
plotreg(Ytrain, Ytrain_predict,valid, valid_predict,ds_test0,ds_DS0,ds_test1,ds_DS1,label='RF',dir_save="/ws_correct_ML/machinelearning/RFplotreg.png")
