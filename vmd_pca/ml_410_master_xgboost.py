...
file = open(r'/ws_correct_ML/vmd_pca/xgb_parameter_win.txt', mode='w')
...
# 模型保存后，从此处开始运行
with open('/ws_correct_ML/vmd_pca/xgbmodel.pickle', 'rb') as f:
    model = pickle.load(f)
print("加载模型成功")
...
data_train=pd.read_csv('/ws_correct_ML/vmd_pca/xgb训练ALL.csv') 
data_valid=pd.read_csv('/ws_correct_ML/vmd_pca/xgb验证ALL.csv')
...
ds_DS.to_csv('/ws_correct_ML/vmd_pca/testandxgb.csv')
ds_DS=pd.read_csv('/ws_correct_ML/vmd_pca/testandxgb.csv')
...
plotreg(Ytrain, Ytrain_predict,valid, valid_predict,ds_test0,ds_DS0,ds_test1,ds_DS1,label='VMD-PCA-XGBoost',dir_save='/ws_correct_ML/vmd_pca/XGBplotreg.png')
